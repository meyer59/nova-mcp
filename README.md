...
The package is not on Packagist yet. Add the local checkout to your application's existing Composer repositories, adjusting the path:

```json
{
  "repositories": [
    {"type": "path", "url": "../nova-mcp"},
...
```bash
composer require nova-mcp/nova-mcp:@dev
php artisan migrate
```

For installation from GitHub, use a VCS repository with URL `https://github.com/meyer59/nova-mcp` instead of the local path, then run `composer require nova-mcp/nova-mcp:dev-master`. While the repository is private, Composer needs GitHub access. Keep your normal Nova Composer credentials configured outside version control.
